...
class AddMovie extends Component {
    ...
    componentDidMount() {
        if (!managerStatus) this.setState({ changePage: true });
        ...
        axios
            ...
            .then(res => {
                if (!res.data.isManager) this.setState({ changePage: true });
            })
            .catch(err => {
                console.log(err);
            });
        ...
        if (this.state.movieInfo.rating === "N/A")
            ...
            return (
                <li>
                    <p>
                        <span>Metascore: </span>
                        {this.state.movieInfo.rating}/100
                    </p>
                </li>
            );
    };
    ...
    loadAddButton = () => {
        if (this.state.movieInfo !== "") {
            if (this.state.movieFromDB.length === 0)
                return (
                    <li>
                        <button onClick={() => this.clickMovieButton()} id="movieButton" className="favorite">
                            Add to the website
                        </button>
                    </li>
                );
                ...
                return (
                    <li>
                        <button
                            onClick={() => this.clickMovieButton()}
                            id="movieButton"
                            className="favorite-color favorite"
                        >
