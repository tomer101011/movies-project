...
export default class NavBar extends Component {
    ...
    componentDidMount() {
        ...
        if (userIdCookie !== undefined) {
            ...
            axios
                .post(url, { userId: userIdCookie })
                .then(result => {
                    if (result.data === "user not found") console.log("not found");
                    else
                        this.setState({
    ...
    login = () => {
        ...
        if (this.state.userName === "" || this.state.password === "") alert("User name or password can't be empty");
        else {
            ...
            axios
                ...
                .then(result => {
                    ...
                    if (result.data === "user not found") alert("User name or password incorrect");
                    else {
    ...
    signUp = () => {
        ...
        if (this.state.newUserName === "" || this.state.newPassword === "" || this.state.confirmPassword === "")
        ...
        else if (this.state.newPassword !== this.state.confirmPassword) alert("You need to confirm the password");
        else {
            ...
            axios
                .post(url, data)
                .then(result => {
                    if (result.data === "user name taken") alert("User name already taken");
                    else {
                        //set a cookie "userId" so after every refresh, the user won't need to log again
                        const cookie = new Cookies();
                        const options = {
    ...
    logOut = () => {
        ...
        if (pageLocation === ROUTES.HOME) window.location.reload(false);
    };
    ...
    addMovieButton = () => {
        if (this.state.isManager)
            return (
                <Link to={{ pathname: ROUTES.ADD_MOVIE }} className="add-movie-style">
                    Add/Remove a Movie
                </Link>
            );
    };
    ...
    addCookieSearch = () => {
        ...
        const pageHref = window.location.href;
        const pageLocation = pageHref.substr(pageHref.indexOf("#") + 1);
        if (pageLocation === ROUTES.ALL_MOVIES) window.location.reload(false);
    };
            ...
            return (
                <ul className="nav navbar-nav navbar-right">
                    <li>
                        ...
                        >
                            Welcome {this.state.loggedUserName}
                        </a>
                        <ul className="dropdown-menu form-wrapper">
                            <li className="center-li">
                                <Link
                                    onClick={() => {
                                        this.addCookieSearch();
                                    }}
                                    to={{ pathname: ROUTES.ALL_MOVIES }}
                                    className="fav-style"
                                >
                                    See All Movies
                                </Link>
                                ...
                                <Link to={{ pathname: ROUTES.HOME }}>
                                    <button onClick={() => this.logOut()} className="logout-text">
                                        Log Out
                                    </button>
                                </Link>
                            </li>
        ...
        return (
            <nav id="navbar" className="navbar navbar-default navbar-expand-lg navbar-light">
                <div className="navbar-header">
                    <Link to={{ pathname: ROUTES.HOME }}>
                        <p onClick={() => this.refreshPage()} className="navbar-brand pTagStyle">
                            Movie<b>Mojo</b>
                        </p>
                    </Link>
                    <button
